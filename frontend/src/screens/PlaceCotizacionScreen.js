...
import { COTIZACION_CREATE_RESET } from '../constants/cotizacionConstants';
...
export default function PlaceOrderScreen(props) {
  ...
  console.log('cambio en plceCotizacion:', cambio);

  const cotizacionCreate = useSelector(state => state.cotizacionCreate);
  const { loading, success, error, cotizacion } = cotizacionCreate;
  ...
  const placeCotizacionHandler = () => {
    dispatch(
      createCotizacion({
        ...cart,
        cotizacionItems: cart.cartItems,
  ...
  useEffect(() => {
    if (success) {
      navigate(`/signin?redirect=/cotizacion/${cotizacion._id}`);
      dispatch({ type: COTIZACION_CREATE_RESET });
    }
  }, [dispatch, cotizacion, navigate, success]);
  console.log('PLACE-COTIZACION cotizacion:', cotizacion);
  return (
    <div>
          ...
          <div className="card resumen">
            <ul>
              ...
              <li>
                ...
                >
                  Grabar Cotizacion (administrador)
                </button>
              </li>
